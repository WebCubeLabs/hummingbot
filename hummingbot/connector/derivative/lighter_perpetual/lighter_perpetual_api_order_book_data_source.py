"""
Lighter Perpetual Order Book Data Source

Extends the spot connector's order book data source for perpetual markets.
"""
...
from hummingbot.connector.derivative.lighter_perpetual.lighter_perpetual_order_book import LighterPerpetualOrderBook
...
class LighterPerpetualAPIOrderBookDataSource(OrderBookTrackerDataSource):
    """Order book data source for Lighter Perpetual exchange."""
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot_msg: OrderBookMessage = LighterPerpetualOrderBook.snapshot_message_from_exchange(
            snapshot,
            snapshot_timestamp,
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        """Create and connect WebSocket assistant with exponential backoff."""
    ...
    async def _subscribe_channels(self, ws: WSAssistant):
        ...
        try:
            for trading_pair in self._trading_pairs:
                ...
                self.logger().info(
                    f"Subscribed to perpetual order book and trade channels for {trading_pair} "
    ...
    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        ...
        order_book_message: OrderBookMessage = LighterPerpetualOrderBook.diff_message_from_exchange(
            data,
            timestamp,
    ...
    async def _parse_order_book_snapshot_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        ...
        snapshot_message: OrderBookMessage = LighterPerpetualOrderBook.snapshot_message_from_exchange(
            data,
            timestamp,
            {"trading_pair": trading_pair}
        )
    ...
    async def _process_single_trade(self, trade_data: Dict[str, Any], message_queue: asyncio.Queue):
        ...
        trade_message: OrderBookMessage = LighterPerpetualOrderBook.trade_message_from_exchange(
            trade_data,
            {"trading_pair": trading_pair}
        )
